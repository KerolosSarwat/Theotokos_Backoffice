...
const UserList = () => {
  ...
  return (
    <div className="user-list">
      ...
      <span class="badge bg-primary">حضانة: {sortedUsers.filter(user => user.level ==="حضانة").length}</span>
      <span class="badge bg-secondary">ابتدائى: {sortedUsers.filter(user => user.level.includes("ابتدائى")).length}</span>
      <span class="badge bg-success">اعدادى: {sortedUsers.filter(user => user.level ==="اعدادى").length}</span>
      <span class="badge bg-danger">ثانوى	: {sortedUsers.filter(user => user.level ==="ثانوى").length}</span>
      <span class="badge bg-warning">جامعة أو خريج: {sortedUsers.filter(user => user.level.includes("خريج")).length}</span>
      <span class="badge bg-info">غير محدد: {sortedUsers.filter(user => user.level ==="").length}</span>
